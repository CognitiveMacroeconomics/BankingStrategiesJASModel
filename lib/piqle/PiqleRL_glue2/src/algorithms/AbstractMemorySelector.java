...
abstract public class AbstractMemorySelector implements ISelector {
	...
	public IAction getChoice(ActionList l) {
	    if(this.freeze) return bestAction(l); 
		if (rouletteWheel)
	...
	/** Auxiliary/debug method : find the best action from an ActionList */
	public IAction bestAction(ActionList l) {
	    IState s=l.getState(); 
		Iterator<IAction> iterator = l.iterator();
		if(l.size()==0) return null;
		IAction meilleure = iterator.next();
	...
	else s+="Exponential alpha decay "+alphaDecayPower+"\n"; 
