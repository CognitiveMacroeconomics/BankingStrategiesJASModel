...
public class EligibleTiles implements Serializable{
    ...
    private static double minTrace=1e-9; 
	...
	boolean bool=false; 
	HashMap<Tile,Double> storeProv=new HashMap<Tile,Double>(); 
	Set keys=store.keySet(); 
	Iterator c=keys.iterator(); 
	while(c.hasNext()){
	    Tile t=(Tile)c.next(); 
	    ...
	    //if(gen.nextDouble()<0.001) System.out.println(u*factor);
	    // if(u*factor>minTrace)
		storeProv.put(t,new Double(u*factor)); 
	}
	this.store=storeProv;
