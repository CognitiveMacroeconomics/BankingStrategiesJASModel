...
public class PiqleAgentTilingRead implements Agent 
{
	...
	this.guru=(TDFASelector)this.readAlgo("/tmp/save9000");

	TileAbleEnvironmentRL universe=(TileAbleEnvironmentRL)this.guru.getWorld(); 
	StateRL.setWorld(universe); 

	//guru.setEpsilon(this.epsilon); 
	guru.setGeometricAlphaDecay();
	guru.setAlpha(0.0001); // TODO : mettre 1e-4 
	//guru.setTau(0.5);
	guru.setGamma(0.9);
	guru.setDecay(0.99999);
	guru.setBoltzmann(); 
	//guru.setLambda(0.6);
    ...
    {
	if(this.episodeNumber%1000==0) {this.saveAlgo("/tmp/savebis"+episodeNumber); }
	...
	if(this.episodeNumber>=0){
	    ...
	    System.out.println("\t\t "+this.moyenne/facteur+" "+this.globalReward/(this.episodeNumber+1)+" "+guru.getAlpha()); 
	}
