...
*/
    ...
    public double getSE(){return ErreurTotale;}

    /** Get quadratic error at output i.*/
    public double getSE(int i){return Erreur[i]; }

    /** Mean Squared Error.*/
    public double getMSE(){return ErreurTotale/nbOutputs;}
...
*/
    ...
    */
	...
	if(Erreur==null) Erreur=new double[nbOutputs]; 
	ErreurTotale=0.0;
	...
	// Quadratic error computation.
		...
		Erreur[i]+=0.5*(receive[nbLayers-1][i]-OutputSample[i])*(receive[nbLayers-1][i]-OutputSample[i]); 
		ErreurTotale+=0.5*(receive[nbLayers-1][i]-OutputSample[i])*(receive[nbLayers-1][i]-OutputSample[i]); 
	    }
    ...
    */
	...
	for(int j=0;j<epoch;j++){
	    ErreurTotale=0; 
	    Erreur=new double[nbOutputs]; 
	    ArrayList grosSac=new ArrayList(); 
